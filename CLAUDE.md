...
11. **Trunk-based workflow — no feature branches on origin.** As of 2026-05-16 the canonical Rally remote has exactly three branches: `main`, `release`, and any agent's currently-active local working branch. Don't push new long-lived `feature/*`, `chore/*`, or `claude/*` branches to origin. Commit on your local branch, push to `main` directly: `git push origin <your-local-branch>:main`. If you must push your branch as a backup ref while iterating, delete it from origin the moment its tip lands on `main`. We just deleted 11 orphan branches; don't be the one to start the proliferation again. The user's explicit rule: "no ton of worktrees hanging."
