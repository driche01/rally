...
11. **Trunk-only on origin — `main`, `release`, and your active local branch are the only branches that should exist.** 2026-05-16 sweep deleted 11 orphan `claude/*`, `perf/*`, `chore/*` branches. Going forward: do work on a local branch in your worktree, push it straight to `main` (`git push origin <local-branch>:main`), and do NOT also push your local branch to origin under a `claude/*` name. The "feature branch as backup ref" pattern is what created the previous mess. If a session ends with your local branch still ahead of `main`, push the diff to `main` first, then it's safe for the branch to die with the worktree.
